...
async def test_import_radarr_movies_new_movie(
    client, test_session, override_get_session, monkeypatch
):
...
@pytest.mark.asyncio
async def test_import_radarr_movies_existing_movie(
    client, test_session, override_get_session, monkeypatch
):
    # First, insert an existing movie manually
    existing_media = Media(type=MediaType.MOVIE, title="Existing Movie", release_date=None)
...
@pytest.mark.asyncio
async def test_import_radarr_movies_invalid_data(
    client, test_session, override_get_session, monkeypatch
):
